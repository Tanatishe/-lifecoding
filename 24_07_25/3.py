...
if __name__ == "__main__":
    ...
    assert (
        target_25 == -1
    ), f"Элемент 25 не существует, должен вернуть -1, получено: {target_25}"

    # Тест 2: Проверка lower_bound
    assert lb_7 == 3, f"Lower bound для 7 должен быть 3 (элемент 8), получено: {lb_7}"
    assert (
        lb_10 == 4
    ), f"Lower bound для 10 должен быть 4 (элемент 10), получено: {lb_10}"

    # Тест 3: Проверка upper_bound
    assert ub_7 == 3, f"Upper bound для 7 должен быть 3 (элемент 8), получено: {ub_7}"
    assert (
        ub_10 == 5
    ), f"Upper bound для 10 должен быть 5 (элемент 12), получено: {ub_10}"

    # Тест 4: Проверка количества элементов в диапазоне
    assert (
        range_count_5_15 == 5
    ), f"В диапазоне [5, 15] должно быть 5 элементов, получено: {range_count_5_15}"
    assert (
        range_count_10_20 == 6
    ), f"В диапазоне [10, 20] должно быть 6 элементов, получено: {range_count_10_20}"
    ...
    assert (
        exists_25 == False
    ), f"Элемент 25 не должен существовать, получено: {exists_25}"
    ...
    print(
        f"Lower bound для 7: {lb_7} (элемент {arr[lb_7] if lb_7 < len(arr) else 'N/A'})"
    )
    print(
        f"Upper bound для 10: {ub_10} (элемент {arr[ub_10] if ub_10 < len(arr) else 'N/A'})"
    )
