...
if __name__ == "__main__":
    ...
    inorder, preorder, postorder, level_order, depth, total_sum, leaves, path_to_7, balanced, widest = result
    
    # Тест 1: Проверка inorder обхода
    expected_inorder = [1, 3, 5, 7, 9, 10, 12, 15, 18]
    assert inorder == expected_inorder, f"Inorder должен быть {expected_inorder}, получено: {inorder}"
    
    # Тест 2: Проверка preorder обхода
    expected_preorder = [10, 5, 3, 1, 7, 9, 15, 12, 18]
    assert preorder == expected_preorder, f"Preorder должен быть {expected_preorder}, получено: {preorder}"
    
    # Тест 3: Проверка postorder обхода
    expected_postorder = [1, 3, 9, 7, 5, 12, 18, 15, 10]
    assert postorder == expected_postorder, f"Postorder должен быть {expected_postorder}, получено: {postorder}"
    
    # Тест 4: Проверка level order обхода
    expected_level_order = [[10], [5, 15], [3, 7, 12, 18], [1, 9]]
    assert level_order == expected_level_order, f"Level order должен быть {expected_level_order}, получено: {level_order}"
    ...
    assert path_to_7 == expected_path, f"Путь к узлу 7 должен быть {expected_path}, получено: {path_to_7}"
    
    # Тест 9: Проверка сбалансированности
    assert balanced == True, f"Дерево должно быть сбалансированным, получено: {balanced}"
    
    # Тест 10: Проверка самого широкого уровня
    assert widest == 4, f"Самый широкий уровень должен содержать 4 узла, получено: {widest}"
